...
Supports two modes (controlled via --use-keypoints flag):
  RGB mode:      webcam frame -> ACT (with vision backbone) -> robot action
  Keypoint mode: webcam frame -> MediaPipe -> keypoints -> ACT (state-only) -> robot action
...
Usage:
    ...
    # Keypoint mode: pose estimation + state-only policy
    ...
    # Keypoint mode with custom landmark set
...
def draw_overlay(frame, action, state, fps, step, use_keypoints=False):
    ...
    mode_label = "KEYPOINT" if use_keypoints else "RGB"
...
def run_webcam(policy, preprocessor, postprocessor, robot, camera_id, fps_target, device,
               use_keypoints=False, pose_estimator=None):
    ...
    mode_str = "KEYPOINT (pose estimation -> state-only ACT)" if use_keypoints else "RGB (vision backbone ACT)"
    ...
    try:
        while True:
            ...
            if use_keypoints and pose_estimator:
                draw_skeleton(frame, pose_estimator)

            display = draw_overlay(frame, current_action, robot_state, fps_actual, step, use_keypoints)
    ...
    if pose_estimator:
        failures = pose_estimator.detection_failure_count
        if failures > 0:
            print(f"Pose detection failures: {failures}/{step} ({failures / max(step, 1) * 100:.1f}%)")
...
def run_video(policy, preprocessor, postprocessor, video_path, output_path, device,
    ...
    while True:
        ...
        if use_keypoints and pose_estimator:
            draw_skeleton(frame, pose_estimator)
        display = draw_overlay(frame, current_action, robot_state, fps, step, use_keypoints)
        if writer:
...
def main():
    ...
    parser.add_argument("--landmarks", default="upper_body",
                        ...
                        help="Landmark preset for keypoint mode")
    parser.add_argument("--no-velocity", action="store_true", help="Disable velocity features (keypoint mode)")
    parser.add_argument("--no-acceleration", action="store_true", help="Disable acceleration features (keypoint mode)")
    parser.add_argument("--smoothing", type=float, default=0.3, help="EMA smoothing alpha (keypoint mode)")
    ...
    if args.use_keypoints:
        ...
        print(f"Keypoint mode: {args.landmarks} preset, {pose_estimator.feature_dim} features")
